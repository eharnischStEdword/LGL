...
// ─── TEMP DEBUG: April data diagnostic ───
app.get("/api/debug/april-summary", requireAuth, async (req, res) => {
  try {
    const results = {};
    ...
    if (LGL_API_KEY) {
      ...
      for (let page = 0; page < 50; page++) {
        ...
        const resp = await fetch(`${LGL_API_BASE}/gifts/search.json?${p}`, {
          headers: { Authorization: `Bearer ${LGL_API_KEY}` },
        });
        if (!resp.ok) {
          results.apiError = `API ${resp.status}: ${(await resp.text()).slice(0, 200)}`;
          break;
        }
        const data = await resp.json();
        allApiGifts.push(...(data.items || []));
        if (offset + (data.items || []).length >= (data.total_items || 0)) {
          results.apiTotalItems = data.total_items;
          break;
        }
        offset += 100;
      }
      ...
      // Offertory specifically
      const offertoryGifts = allApiGifts.filter(g => (g.fund_name || "").toLowerCase() === "offertory");
      results.apiAprilOffertory = {
        count: offertoryGifts.length,
        total: `$${offertoryGifts.reduce((s, g) => s + parseFloat(g.received_amount || 0), 0).toFixed(2)}`,
        gifts: offertoryGifts.slice(0, 10).map(g => ({
          date: g.received_date,
          amount: g.received_amount,
          donor: g.constituent_id,
          updated: g.updated_at,
        })),
      };
    } else {
      results.apiError = "No LGL_API_KEY configured";
    }

    // 2. Check permanent link report date
    const resp = await fetch(LGL_OFFERTORY_URL);
    const cd = resp.headers.get("content-disposition") || "";
    const dateMatch = cd.match(/(\d{4}-\d{2}-\d{2})/);
    results.permanentLink = {
      reportDate: dateMatch ? dateMatch[1] : "(not found in header)",
      contentDisposition: cd.slice(0, 200),
      status: resp.status,
    };

    // 3. Count April rows in permanent link
    const buf = await resp.arrayBuffer();
    const ct = resp.headers.get("content-type") || "";
    const rows = parseSpreadsheetServer(buf, ct);
    const headers = rows.length > 0 ? Object.keys(rows[0]) : [];
    const { dateCol, amountCol, fundCol } = detectColumnsServer(headers);
    let plAprilCount = 0, plAprilTotal = 0;
    if (dateCol && amountCol && fundCol) {
      for (const row of rows) {
        const f = String(row[fundCol] || "").trim().toLowerCase();
        if (f !== "offertory") continue;
        const d = normalizeDateForDedup(row[dateCol]);
        if (d.startsWith("2026-04")) {
          plAprilCount++;
          plAprilTotal += parseFloat(String(row[amountCol] || "0").replace(/[$,]/g, "")) || 0;
        }
      }
    }
    results.permanentLinkAprilOffertory = {
      count: plAprilCount,
      total: `$${plAprilTotal.toFixed(2)}`,
      totalRowsInReport: rows.length,
    };
